...
module.exports = ({aran, argm, antena}) => {
  ...
  advice.invoke = ($$object, $$key, array$$value, serial) => {
    ...
    if (aran.root(serial).alias !== "server" && name === "CanvasRenderingContext2D")
      antena.request("POST", "/log-info", {}, "At "+location(serial)+", context2d."+access.release(access.membrane.leave($$key))+" was called with: "+JSON.stringify(array$$value.map(jsonify), null, 2));
    antena.request("POST", "/log-important", {}, "Invoking", name, access.release(access.membrane.leave($$key)), location(serial));
    if ((aran.root(serial).alias !== "server" && name === "r") || (aran.root(serial).alias === "server" && name === "Socket")) {
      ...
      if (key === "emit") {
        antena.request("POST", "/push", {}, serialize(array$$value[1]));
      } else if (key === "on") {
        const $callback = access.membrane.leave(array$$value[1]);
        array$$value[1] = access.membrane.enter(function () {
          const [status, message, headers, body] = antena.request("GET", "/pop", {}, "");
